...
def insert_event(spvalues):
   ...
   try:
      ...
      if event_counter % 100 == 0:
        conn.commit()
        event_counter_success += 100
        print("Committed 100 , total {}".format(event_counter_success))

   except (Exception, psycopg2.DatabaseError) as error:
      print('Error while inserting an event into PostgreSQL')
      print(error)
      conn.rollback()
...
def lambda_handler(event, context):
    ...
    try:
        ...
        print('Reading {} lines {} from {}'.format(len(lines), file_key, bucket_name))
        #print('lines:')
        #print(lines)
        ...
        print("EVENTS {} SUCCESS {}".format(event_counter, event_counter_success))
        conn.commit()
        conn.close()             
    except (Exception, psycopg2.DatabaseError) as error:
        print('Error while inserting an event into PostgreSQL')
